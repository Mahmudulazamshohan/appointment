...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.services.update", [$service->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label>{{ trans('cruds.service.fields.display_category') }}</label>
                <select class="form-control {{ $errors->has('display_category') ? 'is-invalid' : '' }}" name="display_category" id="display_category">
                    <option value disabled {{ old('display_category', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Service::DISPLAY_CATEGORY_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('display_category', $service->display_category) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('display_category'))
                    <span class="text-danger">{{ $errors->first('display_category') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.service.fields.display_category_helper') }}</span>
            </div>
            <div class="form-group">
