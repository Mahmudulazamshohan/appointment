    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.appointments.update", [$appointment->id]) }}" enctype="multipart/form-data">
                    ...
                    @else
                        ...
                        <select class="form-control select2 {{ $errors->has('specialist') ? 'is-invalid' : '' }}" name="specialist_id" id="specialist_id">
                            @foreach($specialists as $id => $specialist)
                                <option value="{{ $id }}" {{ ($appointment->specialist ? $appointment->specialist->id : old('specialist_id')) == $id ? 'selected' : '' }}>{{ $specialist }}</option>
                            @endforeach
                        </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('booking_date') ? 'is-invalid' : '' }}" type="text" name="booking_date" id="booking_date" value="{{ old('booking_date', $appointment->booking_date) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $appointment->customer->name ?? '') }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="text" name="email" id="email" value="{{ old('email', $appointment->customer->email ?? '') }}" required>
                @if($errors->has('email'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('phone') ? 'is-invalid' : '' }}" type="text" name="phone" id="phone" value="{{ old('phone', $appointment->customer->phone ?? '') }}" required>
                @if($errors->has('phone'))
